...
def recovery():
    ...
    # You might want to restart the bubble check or reset the game state here
    # For example:
    startscript()
...
def checkforbubbles():
    ...
    timeout = 20  # 60 seconds timeout for inactivity

    while True:
        if time.time() - last_action_time > timeout:
            print("Timeout reached, initiating recovery...")
            recovery()
            return  # Optional: return if you handle restart in recovery
...
def automatedfishing():
    ...
    timeout = 30  # Shorter timeout during fishing actions
